(function (w, d) {
  ...
  window.zaraz._al = function (bW, bX, bY) {
    w.zaraz.listeners.push({ item: bW, type: bX, callback: bY });
  ...
  zaraz.f = async function (cE, cF) {
    const cG = { credentials: "include", keepalive: !0, mode: "no-cors" };
    if (cF) {
      cG.method = "POST";
      cG.body = new URLSearchParams(cF);
      cG.headers = { "Content-Type": "application/x-www-form-urlencoded" };
    }
  ...
  !(function (cI, cJ, cK, cL, cM, cN) {
    ...
    function cP(cS, cT, cU, cV) {
      return (
        (cT && cJ.getElementById(cT)) ||
          ((cV = cJ.createElement(cS || "SCRIPT")),
          cT && (cV.id = cT),
          cU && (cV.onload = cU),
          cJ.head.appendChild(cV)),
        cV || {}
      );
    }
    (cN = /p/.test(cJ.readyState)),
      cI.addEventListener("on" + cK in cI ? cK : "load", function () {
        for (cN = 1; cM[0]; ) cO(cM.shift(), cM.shift());
      }),
      (cO._ = cP),
      (cI.defer = cO),
      (cI.deferscript = function (cW, cX, cY, cZ) {
        cO(function () {
          cP("", cX, cZ).src = cW;
        }, cY);
      });
  ...
  zaraz.track = async function (cc, cd, ce) {
    return new Promise((cf, cg) => {
      ...
      Object.keys(zaraz.pageVariables).forEach(
        (cl) => (ch.data[cl] = JSON.parse(zaraz.pageVariables[cl]))
      );
      Object.keys(zaraz.__zcl).forEach(
        (cm) => (ch.data[`__zcl_${cm}`] = zaraz.__zcl[cm])
      );
  ...
  zaraz.set = function (cp, cq, cr) {
    ...
    if (void 0 !== cq) {
      cr && "session" == cr.scope
        ? sessionStorage && sessionStorage.setItem(prefixedKey, cq)
        : cr && "page" == cr.scope
        ? (zaraz.pageVariables[cp] = cq)
        : localStorage && localStorage.setItem(prefixedKey, cq);
      zaraz.__watchVar = { key: cp, value: cq };
    }
  };
  for (const { m: ct, a: cu } of zarazData.q.filter(({ m: cv }) =>
    ["debug", "set"].includes(cv)
  ))
    zaraz[ct](...cu);
  for (const { m: cw, a: cx } of zaraz.q) zaraz[cw](...cx);
  delete zaraz.q;
  delete zarazData.q;
  zaraz.fulfilTrigger = function (bv, bw, bx, by) {
    ...
    zaraz.track(
      "__zarazEmpty",
      { ...bx, __zarazClientTriggers: zaraz.__zarazTriggerMap[bv] },
      by
    );
  };
  window.dataLayer = w.dataLayer || [];
  zaraz._processDataLayer = (de) => {
    for (const df of Object.entries(de))
      zaraz.set(df[0], df[1], { scope: "page" });
    if (de.event) {
      if (
        zarazData.dataLayerIgnore &&
        zarazData.dataLayerIgnore.includes(de.event)
      )
        return;
      let dg = {};
      for (let dh of dataLayer.slice(0, dataLayer.indexOf(de) + 1))
        dg = { ...dg, ...dh };
      delete dg.event;
  ...
  zaraz._rl = function () {
    w.zaraz.listeners &&
      w.zaraz.listeners.forEach((V) =>
        V.item.removeEventListener(V.type, V.callback)
      );
  ...
  zaraz._c = (eR) => {
    const { event: eS, ...eT } = eR;
    zaraz.track(eS, { ...eT, __zarazClientEvent: !0 });
  };
  ...
  zaraz._p({
    e: [
      ...
      "(function(w,d){;d.cookie=unescape('google-analytics_v4_20ac__engagementStart%3D1687617544313%3B%20Domain%3Dcolorlib.com%3B%20Path%3D/%3B%20Max-Age%3D31536000000');d.cookie=unescape('google-analytics_v4_20ac__counter%3D65%3B%20Domain%3Dcolorlib.com%3B%20Path%3D/%3B%20Max-Age%3D31536000000');d.cookie=unescape('google-analytics_v4_20ac__ga4sid%3D1374159010%3B%20Domain%3Dcolorlib.com%3B%20Path%3D/%3B%20Max-Age%3D1800');d.cookie=unescape('google-analytics_v4_20ac__session_counter%3D1%3B%20Domain%3Dcolorlib.com%3B%20Path%3D/%3B%20Max-Age%3D31536000000');d.cookie=unescape('google-analytics_v4_20ac__ga4%3D5c8a070c-829d-4357-8088-f17f58d8ba15%3B%20Domain%3Dcolorlib.com%3B%20Path%3D/%3B%20Max-Age%3D31536000000');d.cookie=unescape('google-analytics_v4_20ac__let%3D1687617544313%3B%20Domain%3Dcolorlib.com%3B%20Path%3D/%3B%20Max-Age%3D31536000000')})(window, document)",
    ],
    f: [
      [
        "https://stats.g.doubleclick.net/g/collect?t=dc&aip=1&_r=3&v=1&_v=j86&tid=G-SEKJ4E9T4H&cid=5c8a070c-829d-4357-8088-f17f58d8ba15&_u=KGDAAEADQAAAAC%7E&z=6613334",
        {},
      ],
    ],
  });
